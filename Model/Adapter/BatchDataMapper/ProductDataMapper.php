...
class ProductDataMapper implements BatchDataMapperInterface
{
    ...
    public function map(array $documentData, $storeId, array $context = [])
    {
        ...
        foreach ($documentData as $productId => $indexData) {
            ...
            foreach ($productIndexData as $attributeCode => $value) {
                ...
                if (in_array($attributeCode, ['width_overlay', 'height_overlay', 'width_undercounter', 'height_undercounter', 'width_flush', 'height_flush'])) {
                    $this->addConfiguratorAttributes($productIndexData);
                }
                if (in_array($attributeCode, ['sink_width', 'sink_height'])) {
                    continue;
                }
    ...
    private function addConfiguratorAttributes($productIndexData)
    {
        ...
        foreach ($groups as $row) {
            ...
            foreach ($row as $key=>$value) {
                $_value = null;
                if (isset($productIndexData[$value])) {
                    $_value = $productIndexData[$value];
                }
                if (empty($_value)) {
                    //$skip = true;
                } else {
                    $data[$key] = $productIndexData[$value];
                }
            }
        }
        if ((int)$data['width'] > 0) {
            $this->builder->addField('sink_width', (int)$data['width']);
        }

        if ((int)$data['height'] > 0) {
            $this->builder->addField('sink_height', (int)$data['height']);
        }
    }
